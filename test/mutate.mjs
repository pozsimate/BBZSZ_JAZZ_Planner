...
function mutateFixedBands(bands, rng, ops, intensity){
  if(!bands.length) return;
  const n = intensity === 'chaos'
    ? rng.int(Math.ceil(bands.length * 0.4), bands.length)
    : rng.int(1, Math.min(8, bands.length));
  const chosen = new Set(rng.sample(bands, n));
  bands.forEach((b, i) => {
    if(!chosen.has(b)) return;
    if(rng.bool(0.2)){ clearPin(b); ops.push(`unpin BAND${i+1}`); return; }
    pinItem(rng, b);
    ops.push(`pin BAND${i+1} ${b.fixedDay} ${b.fixedStart}–${b.fixedEnd}`);
  });
}

function mutateLockedTeachers(db, bands, rng, ops, intensity){
  if(!bands.length) return;
  const quotaIds = (db.bandQuotas || []).map(q => q.teacherId).filter(Boolean);
  const pool = quotaIds.length && rng.bool(0.7) ? quotaIds : db.refTeachers.map(t => t.id);
  const n = intensity === 'chaos'
    ? rng.int(Math.ceil(bands.length * 0.5), bands.length)
    : rng.int(1, Math.min(10, bands.length));
  rng.sample(bands, n).forEach(b => {
    if(rng.bool(0.2)){ b.teacherId = ''; ops.push('unlock a band teacher'); return; }
    b.teacherId = rng.pick(pool);
    const name = (db.refTeachers.find(t => t.id === b.teacherId) || {}).name || b.teacherId;
    ops.push(`lock band teacher ${name}`);
  });
}
...
function mutateBandQuotas(db, rng, ops, intensity){
  db.bandQuotas = db.bandQuotas || [];
  if(db.bandQuotas.length && (intensity === 'chaos' || rng.bool(0.45))){
    const dropN = rng.int(1, Math.min(4, db.bandQuotas.length));
    db.bandQuotas = rng.sample(db.bandQuotas, db.bandQuotas.length - dropN);
    ops.push(`drop ${dropN} quota row(s)`);
  }
  const listed = new Set(db.bandQuotas.map(q => q.teacherId));
  const available = db.refTeachers.filter(t => !listed.has(t.id));
  const addN = intensity === 'chaos'
  ...
  rng.sample(available, addN).forEach((t, i) => {
    ...
    db.bandQuotas.push({
      id: 'BQ' + (db.bandQuotas.length + 1 + i),
      teacherId: t.id,
      teacher: t.name,
      amount,
    });
...
function mutateLessonFlags(db, rng, ops, intensity){
  ...
  rng.sample(db.lessons, n).forEach(l => {
    if(rng.bool(0.5)){
      l.room321 = !l.room321;
      ops.push(`lesson ${l.id} room321=${l.room321}`);
    }
    if(rng.bool(0.45)){
...
function mutateBandFlags(bands, rng, ops, intensity){
  if(!bands.length) return;
  const n = intensity === 'chaos' ? bands.length : rng.int(1, bands.length);
  rng.sample(bands, n).forEach(b => {
    if(rng.bool(0.5)){
      b.room321 = rng.bool(0.7);
      ops.push(`band room321=${b.room321}`);
    }
    if(rng.bool(0.45)){
      b.duration = rng.pick(DURATIONS);
      if(b.fixedStart) pinItem(rng, b);
      ops.push(`band duration=${b.duration}`);
    }
  });
}

function emptyBand(){
  return {bass:[], drum:[], acc:[], sol:[], teacherId:'', room321:true, duration:90, fixedDay:'', fixedStart:'', fixedEnd:''};
}
...
function randomBands(db, rng, n){
  ...
  const bands = Array.from({length:n}, emptyBand);
  bands.forEach(b => {
    ...
    b.room321 = rng.bool(0.75);
    b.duration = rng.pick(DURATIONS);
  });
  return {bands, excluded:[], appearances:{}, eligibleCount: eligible.length, generated:'random'};
}
...
export function buildScenario(api, seed, rng, profile){
  ...
  if(profile === 'baseline'){
    ...
    families = [rng.pick(FAMILIES.filter(f => f !== 'fixedBands' && f !== 'lockedTeachers' && f !== 'bandFlags'))];
  ...
  const preBandFamilies = new Set(['fixedLessons','breaks','classReservations','bandQuotas','teacherAvail','lessonFlags']);
  families.filter(f => preBandFamilies.has(f)).forEach(f => {
    ...
    else if(f === 'bandQuotas') mutateBandQuotas(db, rng, ops, intensity);
  ...
  api.DB = db;
  const bandCount = profile === 'chaos' ? rng.int(10, 22) : rng.int(12, 18);
  const useRandomRoster = profile !== 'baseline' && rng.bool(profile === 'chaos' ? 0.45 : 0.25);
  const bandsState = useRandomRoster
    ? randomBands(db, rng, bandCount)
    : Object.assign(api.generateBands(bandCount), {generated:'algorithm', bandCount});
  ops.push(`${bandsState.generated} bands × ${bandsState.bands.length}`);

  families.forEach(f => {
    if(f === 'fixedBands') mutateFixedBands(bandsState.bands, rng, ops, intensity);
    else if(f === 'lockedTeachers') mutateLockedTeachers(db, bandsState.bands, rng, ops, intensity);
    else if(f === 'bandFlags') mutateBandFlags(bandsState.bands, rng, ops, intensity);
  });

  api.LAST_BANDS = bandsState;
  return {db, bands: bandsState, ops, families, profile};
}
